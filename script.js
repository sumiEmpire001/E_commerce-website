...
// AI
document.addEventListener("DOMContentLoaded", function () {
  // Get all the anchor elements (links) in the navbar
  const navLinks = document.querySelectorAll(".navbar-nav .nav-link");

  // Get all the buttons on the page
  const buttons = document.querySelectorAll("button");

  // Function to generate instructions based on the element's text content
  function generateInstructions(elementType, elementText) {
    return <p><strong>${elementType}:</strong> ${elementText}</p>;
  }

  // Function to display the instructions on the page
  function displayInstructions(instructionText) {
    const instructionsDiv = document.getElementById("instructions");
    instructionsDiv.innerHTML = instructionText;
  }

  // Function to get all the instructions
  function getAllInstructions() {
    let instructions = "<h2>Instructions:</h2>";

    // Get instructions for navbar links
    navLinks.forEach((link) => {
      const elementType = "Link";
      const elementText = link.textContent;
      instructions += generateInstructions(elementType, elementText);
    });

    // Get instructions for buttons
  ...
  // Call the function to get and display all the instructions
  const allInstructions = getAllInstructions();
  displayInstructions(allInstructions);
});